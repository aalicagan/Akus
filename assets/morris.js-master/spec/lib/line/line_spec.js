(function() {
  describe('Morris.Line', function() {
    ...
    describe('generating column labels', function() {
      ...
      it('should use user-defined formatters', function() {
        var chart, d;
        d = new Date(2012, 0, 1);
        chart = Morris.Line({
          element: 'graph',
          ...
          dateFormat: function(d) {
            var x;
            x = new Date(d);
            return "" + (x.getYear() + 1900) + "/" + (x.getMonth() + 1) + "/" + (x.getDay() + 1);
          }
        });
        return chart.data.map(function(x) {
          return x.label;
        }).should.eql(['2012/1/1', '2012/1/2']);
      });
      it('should use user-defined labels', function() {
        var chart;
        chart = Morris.Line({
          element: 'graph',
          data: [
        ...
        return chart.options.customLabels.map(function(x) {
          return x.label;
        }).should.eql(['label']);
      });
      return it('should use relative x-coordinates', function() {
        var chart;
        chart = Morris.Line({
          element: 'graph',
          data: [
        ...
        return [chart.data[1].x - chart.data[0].x].should.not.equal(1);
      });
    });
